...
const skillCategories = {
	web: {
		...
		icon: CodepenIcon,
		description:  "Building modern, responsive web applications with clean code and optimized performance",
		languages: [
			"HTML",
			"CSS",
			"JavaScript",
		...
		tools: ["Visual Studio Code", "Git", "Github", "Figma", "Vite", "Google Cloud", "Postman","Jira"],
	},
	integration: {
		title: "Frontend Integration Specialist",
		icon: WebhookIcon,
		description: "Ensuring seamless integration of frontend with APIs, backend services, and design systems",
		languages: [
			"React",
			"Next.js",
			"JavaScript",
	...
	return (
		<motion.div
			...
			className={`relative cursor-pointer group p-6 rounded-2xl backdrop-blur-lg border transition-all duration-300 ${
				isSelected
					? "bg-white/20 border-gray-500 border-2 shadow-lg"
					: "bg-white/10 border-gray-300 hover:bg-white/20 hover:border-gray-300"
			}`}
			whileHover={{
				scale: 1.05,
				rotateY: 5,
			...
			{!isSelected && (
				<div className="absolute  inset-0 rounded-2xl shadow-2xl transition-opacity duration-300 opacity-0 group-hover:opacity-50 bg-gradient-to-r from-gray-400/20 to-gray-600/20 blur-xl" />
			)}
				...
				<div>
					...
					<div className="flex flex-wrap justify-center gap-2">
						{skill.languages.map((language, index) => (
							<span
								key={index}
								className="px-3 py-1 text-xs font-medium rounded-full bg-gray-300 text-gray-800 hover:bg-gray-300 transition"
							>
								{language}
							</span>
						))}
